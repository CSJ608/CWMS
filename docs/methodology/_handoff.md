...
## 最近更新：2026-08-29（web 渲染器落地——三端投影看得见）

- **apps/web 三端投影渲染器**（issue #25 已关闭，PR #26）：零依赖 Node 服务 + 单页界面，PC 表格 / 大屏卡片与作业看板 / PDA 扫码模拟器三面板；按钮与扫码驱动真实领域操作（任务机+账本），投影 1.5s 轮询刷新。纯消费既有缝与契约，兑现「换 UI = 换渲染器」。启动：`pnpm web` → http://127.0.0.1:8787。全景：15 包（workspace），ADR-0001~0012，80 测试。

## 上一步：2026-08-29（第二纵切片落地——"新功能即新包"首次可执行验证）
...
1. **#27 Phase 1 自治运行（新会话执行）**：Phase 0 注入测试已通过（报告 #27 评论 5462839250：归属门命中 6/6、幻觉误报 0、五门全 fail 无橡皮图章；夹具分支 collab/phase0-injections 留本地）。开跑方式：新会话说「按 agent-lab README 执行 Phase 1」，从 R1 起 PM 状态机驱动 R1→R4；把 Phase 0 副产物的存量问题（错误截断/无客户端校验/空态占位等）酌情并入 R1/R2 验收细目；判定标准见 #27（人工干预 0 为目标、全票通过=失败）
