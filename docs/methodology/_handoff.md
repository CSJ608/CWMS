...
- **collab 实验二：定时项目回顾**（issue #20）：会话自动化 `automation-0726d4b5-d12b-405b-b140-862615f885ac`（每天 09:00，应人要求从工作日放宽）自动汇总开放 issue/PR + CI 状态 + 24h 合并 + 快照下一步，回帖 #20。硬边界：只读 + 唯一写操作一条评论；MCP 不可用则放弃运行（不退回 gh）。首期基线纪要已发（评论 5454878072）。**待验证**：连续数个工作日纪要准确 → 人评审 → 决定是否升级 #21。注意：自动化随本工作区持久，删除即停。
...
1. **#21 值守注册（新会话一步）**：链路演练已通过（PR #22 红→绿 5 分钟，记录见 #21 评论 5459416228，值守提示词已定稿在评论里）；设计定稿：只修类型红/守卫红、连续 2 败停手、永不推 main。剩余动作=新会话 CronCreate 每 2 小时巡检（本工作区会话一任务上限，值守须独立会话注册）。注册后 #21 进入无人值守验证期，人评审修复质量后判定
