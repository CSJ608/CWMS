...
1. **#27 agent-lab 开跑（待人审定路线图）**：物料已就绪（PR #28：协议/路线图草稿/七份角色 rubric）。人审 roadmap-agent-lab-1 的 4 个条目与门禁矩阵 → 先跑 Phase 0 注入测试（6 个已知问题分支盲审五门，度量命中率/误报）→ 再 Phase 1 自治运行。运行方式：新会话说「按 agent-lab README 执行下一轮」；判定标准见 #27（全票通过=实验失败）
